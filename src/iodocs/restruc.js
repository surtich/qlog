 ...
 function createServerPaths(apiDefinition, skelApp, apiName) {
  ...
  for (var i = 0; i < apiDefinition.endpoints.length; i++) {
   ...
   for (var j = 0; j < endpoint.methods.length; j++) {
    ...
    if (method["hero"] && method["hero"]["require"]) {
     if (Object.prototype.toString.call( method["hero"]["require"] ) !== '[object Array]') {
      method["hero"]["require"] = [method["hero"]["require"]];
     }
     for (var k = 0; k < method["hero"]["require"].length; k++) {
      ...
      skelApp["path"][method["hero"]["require"][k]] = {
       "type": "file",
       "overwrite": "replace",
       "contents": contents
      };
