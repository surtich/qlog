...
hero.init = function (iodocsApisConfig, basedir, f_callback){
	var Restruc = require(path.join(basedir, 'src', 'iodocs/restruc')).Restruc;

	var restruc = new Restruc(basedir, iodocsApisConfig);
		...
		for (var i = 0; i < api.endpoints.length; i++) {
			...
			for (var j = 0; j < endpoint.methods.length; j++) {
				...
				for (var k = 0; k < method.hero.handler.length; k++) {
					registerPath(method.URI, method.HTTPMethod, require(path.join(basedir, 'src', apiName, "path", method.hero.handler[k])));	
				}
